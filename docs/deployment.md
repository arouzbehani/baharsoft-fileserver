...
Production installations should pin an exact version such as `1.0.2`, not
`latest`.

## Install a release on a Linux server

The following example installs version `1.0.2` under
`/opt/baharsoft-fileserver`. Replace the version when installing a newer
release.

```bash
export FILESERVER_VERSION=1.0.2
